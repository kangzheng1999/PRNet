...
class Filter(nn.Module):
    def __init__(self, channels):
        ...
        # self.test = cbam(channels, kernel_size=7)  #cbam
        # self.test = a2(channels, channels,channels)
        # self.test = ACmix(channels, channels)
        self.test = PFblock(channels)
        self.scale = nn.Sequential(
            nn.Conv2d(channels, channels, 1, padding=0),
        )

    def forward(self, x):
        x = self.scale(self.test(self.resnet(x)))
...
class PRNet(nn.Module):
    def __init__(self, config, use_gpu=True):
        ...
        # self.test = cbam(config.net_channels, kernel_size=7)
    ...
    def forward(self, data):
        ...
        # B1NC -> BCN
        input = data['xs'].transpose(1, 3).squeeze(3)  # N*4*num_pts
        x1, x2 = input[:, :2, :], input[:, 2:, :]
        motion = x2 - x1  # N*2*num_pts  坐标差

        pos = x1  # B2N
        grid_pos = self.grid_center(batch_size)  # B2N 生成(batch_size, 2, grid_num * grid_num)的tensor

        pos_embed = self.pos_embed(pos)  # BCN  用x1生成(batch_size, 128, grid_num * grid_num)的tensor
        grid_pos_embed = self.grid_pos_embed(grid_pos)  # BCN  生成(batch_size, 128, grid_num * grid_num)的tensor

        d = self.init_project(
            motion) + pos_embed  # BCN 用motion生成(batch_size, 128, grid_num * grid_num)的tensor，与pos_embed相加
...
def batch_symeig(X):
    ...
    for batch_idx in range(X.shape[0]):
        e, v = torch.linalg.eigh(X[batch_idx], UPLO='L')
        bv[batch_idx] = v
    bv = bv.cuda()
    return bv
